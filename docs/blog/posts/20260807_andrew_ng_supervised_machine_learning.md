...
Some classification applications: determining if email is spam, determining if a transaction is fraudulent, determining if a tumor is malignant. A classification model makes a classification based on the probability of an input being in a category.

Classification predicts discrete categories, while regression predicts real numbers. Here's a question: if I can get a model to learn how to predict real numbers well by learning weights, how can I get it learn to predict categories? Well, one way would be to use linear regression to fit the data, then affixing a decision boundary to the line. But if an outlier is added to the training data, this would update the linear regression model. This isn't ideal because the outlier shouldn't change what the decision boundary is differentiating. So linear regression isn't ideal for a classification algorithm. Instead, I will look for a better approach. Aha! It's called logistic regression.

### Logistic regression
Despite having "regression" in the name, logistic regression is used for classification! According to Ng, it is one of the most widely used classification algorithms in the world. In classification, and specifically binary classification, the model determines which of two categories a datapoint belongs to. A sigmoid function $f(z) = 1 / (1 + e^{-z})$ overcomes the pitfall of linear regression for classification because it is unaffected by outliers. No matter what input value it takes, the results are bounded between 0 and 1. So, what happens when the outputs of a linear regression model, $z$, are fed into a sigmoid function? The sigmoid function transforms/normalizes those outputs to a range between 0 and 1. The output of the sigmoid function can be interpreted as the probability that the input is classified as 1 or 'positive'. When the sigmoid function is applied to the output of a linear regression function, it is called a logistic regression.

By the way, $z$ can be a scalar or a vector. When $z$ is a vector, the sigmoid function is calculated for each input. Each input gets its own prediction. Plotting sigmoid function for values between -10 and 10 inclusive shows the behavior of the output. 
![Figure 1](../../assets/Figure%201.png)

Okay, you might say, but this all seems contrived. Why do I need to affix my decision boundary to a model in the first place? For example, if multiple linear regression is predicting the size of a tumor, why would I need to feed the predicted tumor sizes into the sigmoid function? Can't I just designate tumors above 2cm in diameter as malignant and all others as benign? Yes, but if malignancy could be determined by tumor diamter alone, this wouldn't be an example where machine learning classification is useful. ML classification is useful when the determination depends on multiple features and it isn't easy to arrive at the conclusion. In other words, ML classification is used when determining the decision boundary isn't easy. The classification is learning the decision boundary from the training examples.

#### Decision Boundaries
After feeding the regression into the sigmoid function, we want to decide on the boundary that separates a positive classification from a negative one. We know that the sigmoid function value depends on $z$, its input. But how do we express the boundary in terms of the input function? Well, if the decision boundary on the sigmoid function is determined to be $0.3$, then the input function must be whatever value of z that results in $g(z) = 0.3$. Recall that z is a linear regression model. The model will learn the weights it needs to determine which side of the decision boundary some new data belongs to. For the model to learn the proper weights, there needs to be a metric that determines how well it has learned the proper model. This is the role of the classification cost function.

#### Cost function for logistic regression
In linear regression, the MSE cost function was used because its convex nature allowed the algorithm to converge to a minimum. In logistic regression, the same cost function doesn't work as well because the logistic function is nonlinear. This leads to the cost function exhibiting several local minima, which is not ideal for the gradient descent algorithm.